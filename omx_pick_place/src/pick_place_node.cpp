...
{
public:
  PickPlaceNode() : Node("omx_pick_place_node")
  {
    // 필요 시 시뮬 시간
    ...
    // pick실행 토픽 타겟 좌표 float 배열
    target_pose_sub_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
      "/target_pose",
      rclcpp::QoS(10),
...
private:
  void onPlaceCommand(const std_msgs::msg::String::SharedPtr)
  {
    ...
    std::thread([self]() {
      ...
      try {
        ...
        ok = false;
        RCLCPP_ERROR(self->get_logger(), "Place failed: %s", e.what());
      }
  ...
  void onTargetPose(const std_msgs::msg::Float32MultiArray::SharedPtr msg)
  {
    // NOTE: MultiThreadedExecutor에서 콜백은 병렬 실행될 수 있으므로
    //       재진입 방지를 위해 atomic flag 사용.
    if (busy_.exchange(true)) {
      RCLCPP_WARN(get_logger(), "Pick or Place is already running. Ignoring /target_pose.");
      return;
    }

    // 좌표 데이터 확인
    if (msg->data.size() < 3){
      RCLCPP_ERROR(get_logger(), "Received /target_pose with insufficient data (need at least 3 floats)");
      busy_.store(false);
      return;
    }
    ...
    // IMPORTANT:
    // /target_pose 구독 콜백에서 MoveIt execute(), 서비스 호출 wait 등
    // 시간이 오래 걸리는 작업을 바로 수행하면, 같은 executor가 처리해야 하는
    // MoveIt action 응답(GoalResponse/Result) 콜백이 지연/꼬이면서
    // "unknown goal response" -> abort/timeout로 이어질 수 있습니다.
    // 그래서 실제 Pick&Place는 별도 스레드에서 실행하고, 콜백은 즉시 반환합니다.
    ...
    std::thread([self]() {
      ...
      try {
        ...
        ok = false;
        RCLCPP_ERROR(self->get_logger(), "Pick failed: %s", e.what());
      }
  ...
  geometry_msgs::msg::PoseStamped pregrasp(geometry_msgs::msg::PoseStamped p)
  {
    ...
    const double new_norm = std::max(0.0, norm - 0.04);
    const double scale = new_norm / norm;

    p.pose.position.x = x * scale;
    p.pose.position.y = y * scale;
    p.pose.position.z = z * scale;
...
int main(int argc, char ** argv)
{
  ...
  RCLCPP_INFO(node->get_logger(), "Waiting for /target_pose ...");
