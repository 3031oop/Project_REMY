				...
				try:
					...
					if match:
						...
						if pin_num in LED_PINS:
							idx = LED_PINS.index(pin_num)
							GPIO.output(LED_PINS[idx],True)
							GPIO.output(VIB_PINS[idx],True)
							print(f">>> {pin_num}번 LED&진동 ON")
						else:
							print(f">>> 경고: {pin_num}번은 LED_PINS 리스트에 없습니다.")
				except: pass

			elif "DANGER" in rstr:
				if 15 in LED_PINS:
					idx_15 = LED_PINS.index(15)
					GPIO.output(LED_PINS[idx_15], True)
					GPIO.output(VIB_PINS[idx_15], True)
				# GPIO.output(15, True)
				print(">>> [위험] 칼날 접근! 15번 LED 점등")
			elif "DETECTED" in rstr:
				for i in range(len(LED_PINS)):
					GPIO.output(LED_PINS[i], True)
					GPIO.output(VIB_PINS[i], True)
				# for pin in LED_PINS:
