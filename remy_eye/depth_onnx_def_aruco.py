...
def detection_box(tools_dect, hand_dect, current_frame, depth_frame, intr):
    ...
    global prev_led_msg, prev_event, danger_cnt, detected_cnt 
    global danger_start_time, detected_start_time
    if len(blade_xy)  > 0:
    ...
    try:
        ...
        if flag_target:
            if detected_start_time == 0:
                detected_start_time = time.time()
            
            if (time.time() - detected_start_time) >= CONFIRM_TIME:
