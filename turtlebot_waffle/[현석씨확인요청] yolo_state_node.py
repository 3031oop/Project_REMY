...
class FallingObjectSmartReturn(Node):
    def __init__(self):
        ...
        # 상태 보고 토픽
        self.status_pub = self.create_publisher(String, '/waffle_status', 10)
        ...
        if self.state == 'IDLE':
            ...
            if self.patrol_sub_state == 'MOVE':
                ...
                if dist < 1.0:
                    ...
                    self.rotate_end_time = time.time() + 21.0
            ...
            if target['cx'] is not None:
                ...
                if self.stop_count > 5:
                    self.publish_status("detect")

                    self.target_dist = target['dist']
                    self.last_error = (self.width / 2) - target['cx']
            ...
            if moved < max(0.0, self.target_dist - 0.3):
                ...
                self.publish_status("depart")
                self.state = 'WAIT'
                self.prev_sent_state = None

        elif self.state == 'WAIT':
            self.publish_status("wait_return")

            if self.wait_start_time is not None and (time.time() - self.wait_start_time > 5.0):
                self.publish_status("force_return")
                self.state = 'RETURN'
                self.prev_sent_state = None
            ...
            if abs(diff) > 0.05:
                ...
                self.send_control(0.0, 0.0)
                self.get_logger().info("ALIGN_FINAL -> IDLE")
                self.state = 'IDLE'
                self.prev_sent_state = None
